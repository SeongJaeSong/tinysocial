...
describe('[Mutation]', () => {
  test('signInWithGoogle', async () => {
    ...
    const res = await Mutation.signInWithGoogle(
        {},
  ...
  test('signUpWithGoogle', async () => {
    signUpWithGoogle.mockReturnValueOnce({
      success: true,
      message: 'Success',
      token: 'TOKEN',
      user: {id: 1},
    });
    const res = await Mutation.signUpWithGoogle(
        {},
        {googleId: 'test@test.com'},
        mockContext,
    );
  ...
  test('signIn', async () => {
    ...
    const res = await Mutation.signIn(
        {},
        {email: 'test@test.com', pw: 'testpw'},
        mockContext,
    );
    expect(res).toEqual({
      success: true,
  ...
  test('signUp', async () => {
    ...
    const res = await Mutation.signUp(
        {},
        {
          email: 'test@test.com',
          firstName: 'firstname',
  ...
  test('emailValidate', async () => {
    emailValidate.mockReturnValueOnce({
      success: true,
      message: 'Success',
      token: 'TOKEN',
      user: {id: 1},
    });
    const res = await Mutation.emailValidate(
        {},
        {token: 'TOKEN'},
        mockContext,
    );
  ...
  test('logOut', async () => {
    expect(Mutation.logOut({}, {}, mockContext)).rejects.toThrow(
        'Not implemented yet',
    );
  });
  test('createOrModifyOfReview', async () => {
    createOrModifyOfReview.mockReturnValueOnce({userId: 1, eventId: 1});
    const res = await Mutation.createOrModifyReview(
        {},
        {
          eventId: 1,
          title: 'testTitle',
